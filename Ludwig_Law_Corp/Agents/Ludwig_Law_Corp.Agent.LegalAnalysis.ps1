...
if (Test-Path $AgentCommPath) {
    ...
    & $AgentCommPath -AgentName "Ludwig_Law_Corp.Agent.LegalAnalysis" -Division "Ludwig_Law_Corp" -InitializeNetwork

    # Communication functions for agent use
    function Send-AgentMessage {
        param([string]$To, [string]$Type, [string]$Content, [string]$Priority = "Normal")
        & $AgentCommPath -AgentName "Ludwig_Law_Corp.Agent.LegalAnalysis" -TargetAgent $To -MessageType $Type -MessageContent $Content -Priority $Priority -SendMessage
    }

    function Receive-AgentMessages {
        return & $AgentCommPath -AgentName "Ludwig_Law_Corp.Agent.LegalAnalysis" -ReceiveMessages
    }

    function Broadcast-Message {
        param([string]$Type, [string]$Content, [string]$Priority = "Normal")
        & $AgentCommPath -AgentName "Ludwig_Law_Corp.Agent.LegalAnalysis" -MessageType $Type -MessageContent $Content -Priority $Priority -Broadcast
    }
    ...
    Write-Host "ðŸ”— Agent communication system initialized for Ludwig_Law_Corp.Agent.LegalAnalysis" -ForegroundColor Cyan
}
