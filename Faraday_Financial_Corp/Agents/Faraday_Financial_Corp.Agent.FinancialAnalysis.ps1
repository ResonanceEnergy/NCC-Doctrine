...
if (Test-Path $AgentCommPath) {
    ...
    & $AgentCommPath -AgentName "Faraday_Financial_Corp.Agent.FinancialAnalysis" -Division "Faraday_Financial_Corp" -InitializeNetwork

    # Communication functions for agent use
    function Send-AgentMessage {
        param([string]$To, [string]$Type, [string]$Content, [string]$Priority = "Normal")
        & $AgentCommPath -AgentName "Faraday_Financial_Corp.Agent.FinancialAnalysis" -TargetAgent $To -MessageType $Type -MessageContent $Content -Priority $Priority -SendMessage
    }

    function Receive-AgentMessages {
        return & $AgentCommPath -AgentName "Faraday_Financial_Corp.Agent.FinancialAnalysis" -ReceiveMessages
    }

    function Broadcast-Message {
        param([string]$Type, [string]$Content, [string]$Priority = "Normal")
        & $AgentCommPath -AgentName "Faraday_Financial_Corp.Agent.FinancialAnalysis" -MessageType $Type -MessageContent $Content -Priority $Priority -Broadcast
    }
    ...
    Write-Host "ðŸ”— Agent communication system initialized for Faraday_Financial_Corp.Agent.FinancialAnalysis" -ForegroundColor Cyan
}
