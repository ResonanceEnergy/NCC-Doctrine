...
if ($Audit) {
    ...
    Write-Host "🔥 LFG! LAUNCH PROCEDURES AUDIT RESULTS 🔥" -ForegroundColor Magenta
    ...
    if ($auditResults.critical_issues.Count -gt 0) {
        ...
        foreach ($issue in $auditResults.critical_issues) {
            ...
            foreach ($issueDetail in $issue.issues) {
                Write-Host "    • $issueDetail" -ForegroundColor Yellow
            }
        }
    }
...
if ($Activate -or $MaxEfficiency -or $MaxProduction) {
    ...
    Write-Host "🚀 LFG! PROTOCOLS FULLY ACTIVATED 🚀" -ForegroundColor Magenta
