...
function Invoke-DeptHeadAnalysis {
    ...
    Write-Host "`n👨‍💼 Department Head Analysis Process..." -ForegroundColor Cyan
...
function Get-DepartmentAgents {
    ...
    Write-Host "`n🤖 Identifying department agents..." -ForegroundColor Cyan
    ...
    if (!$agents) {
        Write-Host "⚠️ No predefined agents for $DepartmentName, creating generic agents" -ForegroundColor Yellow
    ...
    foreach ($agent in $agents) {
        $agentPath = Join-Path $AgentsPath "$agent.json"
        if (Test-Path $agentPath) {
            Write-Host "✓ Agent available: $agent" -ForegroundColor Green
        } else {
            Write-Host "✗ Agent file missing: $agent - Creating..." -ForegroundColor Yellow
            $agentData = @{
                AgentName = $agent
                Department = $DepartmentName
                Status = "Active"
...
function New-AgentTask {
    ...
    Write-Host "`n📝 Creating task for agent: $AgentName" -ForegroundColor Cyan
...
"@
    ...
    Write-Host "📄 Saved to: $taskPath" -ForegroundColor Cyan
    Write-Host "📅 Due date: $($task.DueDate)" -ForegroundColor Yellow
...
function Update-DepartmentDirective {
    ...
    Write-Host "✓ Updated department directive with assigned agents" -ForegroundColor Green
}
...
# Main execution
Write-Host "🏢 NCC Hierarchical Directive System - Department Head Task Creation" -ForegroundColor Magenta
...
Write-Host "`n🏗️ Creating agent tasks..." -ForegroundColor Cyan
