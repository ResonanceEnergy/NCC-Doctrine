...
function Get-DepartmentDirective {
    ...
    if (!(Test-Path $deptPath)) {
        Write-Host "❌ Department directory not found: $deptPath" -ForegroundColor Red
        return $null
    }
    ...
    if (!$DirectiveName) {
        ...
        if ($directiveFiles.Count -eq 0) {
            Write-Host "❌ No directives found for $DepartmentName" -ForegroundColor Red
            return $null
        }
        ...
        if (!$directiveFile) {
            Write-Host "❌ Directive '$DirectiveName' not found in $DepartmentName" -ForegroundColor Red
            return $null
        }
    }
    ...
    try {
        ...
        Write-Host "✓ Loaded directive: $($directive.DirectiveName)" -ForegroundColor Green
        return $directive
    } catch {
        Write-Host "❌ Error loading directive: $_" -ForegroundColor Red
        return $null
    }
}
...
function Get-AgentReports {
    ...
    Write-Host "`n📋 Collecting agent reports..." -ForegroundColor Cyan
    
    $agentReports = @()
    
    if (!$Directive.AssignedAgents -or $Directive.AssignedAgents.Count -eq 0) {
        Write-Host "⚠️ No assigned agents found in directive" -ForegroundColor Yellow
        return $agentReports
    }
    ...
    foreach ($agent in $Directive.AssignedAgents) {
        ...
        Write-Host "🔍 Looking for report from: $agentName" -ForegroundColor Gray
        ...
        if ($reportFiles.Count -gt 0) {
            ...
            try {
                ...
                $agentReports += $report
                Write-Host "✓ Found report: $($report.ReportName)" -ForegroundColor Green
            } catch {
                Write-Host "❌ Error loading report $($latestReport.Name): $_" -ForegroundColor Red
            }
        } else {
            Write-Host "❌ No report found for $agentName" -ForegroundColor Red
        }
    }
...
function New-DepartmentReport {
    ...
    Write-Host "`n📊 Compiling department report..." -ForegroundColor Cyan
    ...
    Write-Host "✓ Generated department report: $reportName" -ForegroundColor Green
...
function Send-MetricsToAX {
    ...
    Write-Host "`n📤 Transmitting department metrics to AX system..." -ForegroundColor Cyan
    ...
    Write-Host "✓ Department metrics transmitted to AX: $metricsFile" -ForegroundColor Green
}
...
function Send-ReportToCEO {
    ...
    Write-Host "`n👔 Sending department report to CEO..." -ForegroundColor Cyan
    ...
    Write-Host "✓ Department report sent to CEO: $messageName" -ForegroundColor Green
    Write-Host "📬 Message saved to: $messagePath" -ForegroundColor Cyan
...
function Show-DepartmentDirectives {
    ...
    Write-Host "`n📋 Available directives for $DepartmentName :" -ForegroundColor Cyan
...
# Main execution
Write-Host "🏢 NCC Hierarchical Directive System - Department Head Report Compilation" -ForegroundColor Magenta
...
if (!$DepartmentName) {
    Write-Host "❓ Please specify -DepartmentName" -ForegroundColor Yellow
...
if ($agentReports.Count -eq 0) {
    Write-Host "⚠️ No agent reports available. Cannot compile department report." -ForegroundColor Yellow
