...
if (Test-Path $AgentCommPath) {
    ...
    & $AgentCommPath -AgentName "GlobalTalentAcquisition.Agent.HROperations" -Division "GlobalTalentAcquisition" -InitializeNetwork

    # Communication functions for agent use
    function Send-AgentMessage {
        param([string]$To, [string]$Type, [string]$Content, [string]$Priority = "Normal")
        & $AgentCommPath -AgentName "GlobalTalentAcquisition.Agent.HROperations" -TargetAgent $To -MessageType $Type -MessageContent $Content -Priority $Priority -SendMessage
    }

    function Receive-AgentMessages {
        return & $AgentCommPath -AgentName "GlobalTalentAcquisition.Agent.HROperations" -ReceiveMessages
    }

    function Broadcast-Message {
        param([string]$Type, [string]$Content, [string]$Priority = "Normal")
        & $AgentCommPath -AgentName "GlobalTalentAcquisition.Agent.HROperations" -MessageType $Type -MessageContent $Content -Priority $Priority -Broadcast
    }
    ...
    Write-Host "ðŸ”— Agent communication system initialized for GlobalTalentAcquisition.Agent.HROperations" -ForegroundColor Cyan
}
