...
if (Test-Path $AgentCommPath) {
    ...
    & $AgentCommPath -AgentName "GlobalLogisticsNetwork.Agent.TransportationCoordination" -Division "GlobalLogisticsNetwork" -InitializeNetwork

    # Communication functions for agent use
    function Send-AgentMessage {
        param([string]$To, [string]$Type, [string]$Content, [string]$Priority = "Normal")
        & $AgentCommPath -AgentName "GlobalLogisticsNetwork.Agent.TransportationCoordination" -TargetAgent $To -MessageType $Type -MessageContent $Content -Priority $Priority -SendMessage
    }

    function Receive-AgentMessages {
        return & $AgentCommPath -AgentName "GlobalLogisticsNetwork.Agent.TransportationCoordination" -ReceiveMessages
    }

    function Broadcast-Message {
        param([string]$Type, [string]$Content, [string]$Priority = "Normal")
        & $AgentCommPath -AgentName "GlobalLogisticsNetwork.Agent.TransportationCoordination" -MessageType $Type -MessageContent $Content -Priority $Priority -Broadcast
    }
    ...
    Write-Host "ðŸ”— Agent communication system initialized for GlobalLogisticsNetwork.Agent.TransportationCoordination" -ForegroundColor Cyan
}
function Initialize-Agent { Write-AgentLog "Initializing Transportation Coordination Agent..."; $AgentConfig.Status = "Initialized"; Write-AgentLog "Transportation Coordination Agent ready" -Level "SUCCESS" }
function Start-AgentOperations { $AgentConfig.Status = "Active"; Write-AgentLog "Transportation Coordination operations started" -Level "SUCCESS" }
