...
function Get-CompanyDirective {
    ...
    if (!$DirectiveName) {
        ...
        if ($directiveFiles.Count -eq 0) {
            Write-Host "❌ No directives found for $CompanyName" -ForegroundColor Red
            return $null
        }
        ...
        if (!$directiveFile) {
            Write-Host "❌ Directive '$DirectiveName' not found" -ForegroundColor Red
            return $null
        }
    }
...
function Invoke-CEOAnalysis {
    ...
    Write-Host "`n🧠 CEO Internalization Process..." -ForegroundColor Cyan
...
function Get-RelevantDepartments {
    ...
    Write-Host "`n🏢 Identifying relevant departments..." -ForegroundColor Cyan
    ...
    if (!$departments) {
        Write-Host "⚠️ No predefined departments for $CompanyName, using generic structure" -ForegroundColor Yellow
    ...
    foreach ($dept in $departments) {
        ...
        if (!(Test-Path $deptPath)) {
            ...
            Write-Host "✓ Department exists: $dept" -ForegroundColor Green
        }
    }
    
    # Flag missing departments to AZ
    if ($missingDepartments.Count -gt 0) {
        Write-Host "🚩 Flagging missing departments to AZ PRIME:" -ForegroundColor Red
...
function New-DepartmentDirective {
    ...
    Write-Host "`n📋 Creating department directive for: $DepartmentName" -ForegroundColor Cyan
...
"@
    ...
    Write-Host "✓ Created department directive: $directiveName" -ForegroundColor Green
    Write-Host "📄 Saved to: $deptDirectivePath" -ForegroundColor Cyan
...
function Show-AvailableDirectives {
    ...
    Write-Host "`n📋 Available directives for $CompanyName :" -ForegroundColor Cyan
...
# Main execution
Write-Host "👑 NCC Hierarchical Directive System - CEO Directive Breakdown" -ForegroundColor Magenta
...
Write-Host "`n🏗️ Creating department directives..." -ForegroundColor Cyan
foreach ($dept in $departments) {
